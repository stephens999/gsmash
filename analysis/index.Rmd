---
title: "hOMEPAGe"
site: workflowr::wflow_site
output:
  workflowr::wflow_html:
    toc: false
editor_options:
  chunk_output_type: console
---



## Normal mean penalty form

* [bound on (log g)''](bound_log_marginal.html)
* [log density of normal mixture](log_gmm_concave.html)
  + [log density of normal mixture, variance ratio](log_gmm_concave_ratio.html)
* [log density of laplace mixture](log_lmm_concave.html)
* [normal mean penalty plots](normal_mean_penalty.html)
* [Solve normal mean problem using optimization - compund penalty](normal_mean_penalized_optimization.html)
* [Solve normal mean problem using optimization - inversion method](normal_mean_penalized_optimization_inversion.html)
* [Solve normal mean problem using optimization - also estimate prior mean](normal_mean_penalized_optimization_prior_mean.html)
* [normal mean penalty plots, when $s^2$ depends on posterior mean](normal_mean_penalty_for_glm.html)
* [normal mean penalty plots, when $s^2$ depends on posterior mean, penalty exlcuding $log(2\pi s^2)/2$ ](normal_mean_penalty_glm_simplified.html)


## Poisson mean 

* [Poisson mean, ash prior, Gaussian posterior]()
* [Poisson mean, ash prior, Gaussian Mixture posterior]()
* [NB JJ lower bound](neg_binom_mean_lower_bound.html)
* [NB PG augmentation](neg_binom_mean_polya_gamma.html)
* [Poisson mean $log(1+exp(\cdot))$ link](pois_mean_log1exp.html)

### Penalized

* [Poisson mean penalized form, compound](pois_mean_penalized.html)
* [Poisson mean penalized form, inversion](pois_mean_penalized_inversion.html)

## Poisson smoothing nugget 

* [The two step procedure for smoothing overdispersed count data](poisson_nugget_two_step.html)



